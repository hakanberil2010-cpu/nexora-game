-- Phase 10: preserve refunds for nullable legacy city resource balances.
-- Only the four credit expressions differ from 018; retain its locks and guard.
BEGIN;
...
BEGIN
  ...
  IF v_refund_now > 0 THEN
    ...
    ELSIF v_offer.give_resource = 'water' THEN
      UPDATE public.cities
         SET water = COALESCE(water, 0) + v_refund_now
       WHERE id = v_city.id;
    ELSE
      UPDATE public.cities
