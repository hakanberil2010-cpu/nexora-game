-- NEXORA - Alliance Region Production Bonuses V1
-- Apply after 036_alliance_region_bonuses_v1.sql.
--
-- Adds the active alliance territory production bonus directly inside the
-- authoritative city-production sync. Existing production, storage, research
-- and multi-building behavior is preserved.
...
BEGIN
  ...
  -- Server-authoritative territory ownership from migration 036.
  -- Client coordinates / alliance claims are never trusted here.
  ...
  ELSE
    v_region_bonus_active := false;
    v_region_bonus_key := NULL;
  END IF;
  ...
  IF v_region_bonus_active THEN
    CASE v_region_bonus_key
      ...
      WHEN 'water_production' THEN
        v_water_rate := v_water_rate * 1.05;
